    ...
    public class RaySensor : Sensor3D
    {
        ...
        protected override Collider[] GetComponentsInSensor()
        {
            RaycastHit[] hits = Physics.RaycastAll(
                transform.position, 
                Vector3.Scale(transform.forward, transform.lossyScale).normalized, 
                _distance, 
                _scanLayer,
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            Gizmos.DrawRay(transform.position, Vector3.Scale(transform.forward, transform.lossyScale).normalized * _distance);
        }
    }
}